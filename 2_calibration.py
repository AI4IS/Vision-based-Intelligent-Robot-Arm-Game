...
if __name__ == "__main__":
    cali = Calibration()
    if not os.path.exists("./save_parms_test/image_to_arm.npy") or not os.path.exists("./save_parms_test/arm_to_image.npy"):
...
#                 self.device.move_to(point[0], point[1], point[2], point[3], wait=True)
#                 # add x offset
#                 arm_cord.T[index][0] = arm_cord.T[index][0] + 45  # +40 因为 aruco marker 的中心点距离end effector 30mm
#
#                 # get the position of the aruco marker
#                 images, center = self.get_aruco_center()
#                 if center is not None:
